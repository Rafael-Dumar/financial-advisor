...
class AutomationScriptsTests(unittest.TestCase):
    ...
    def test_fetch_latest_github_reports_script_contract(self) -> None:
        ...
        self.assertIn("no_valid_current_day_artifact_pair", content)
        self.assertIn("artifact_selection", content)
        self.assertIn("expected_head_sha", content)
        self.assertIn("function ConvertTo-Iso8601UtcString", content)
        self.assertIn("created_at = ConvertTo-Iso8601UtcString $view.createdAt", content)
        self.assertIn("artifact_created_at = ConvertTo-Iso8601UtcString $artifact.created_at", content)
        self.assertIn("report_generated_at = ConvertTo-Iso8601UtcString $generatedAt", content)
        self.assertNotIn("created_at = [string]$view.createdAt", content)
        self.assertNotIn("artifact_created_at = [string]$artifact.created_at", content)
        self.assertNotIn("report_generated_at = [string]$generatedAt", content)
