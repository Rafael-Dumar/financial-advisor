...
$LogPath = Join-Path $LogDir "close-report-$RunStamp.log"
...
try {
    ...
    Copy-Item -LiteralPath $MarkdownReport -Destination (Join-Path $HistoryDir "$BrtDate-close.md") -Force

    if (Test-Path -LiteralPath $HtmlReport) {
        Copy-Item -LiteralPath $HtmlReport -Destination (Join-Path $ReportsDir 'latest.html') -Force
        Copy-Item -LiteralPath $HtmlReport -Destination (Join-Path $HistoryDir "$BrtDate-close.html") -Force
    }

    Write-Host 'report_ready=reports\latest.md'
    Write-Host "history_report=reports\history\$BrtDate-close.md"
